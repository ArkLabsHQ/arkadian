...
```json
{
  ...
  "artifacts_produced": [
    { "path": "research_report.md", "type": "report" }
  ],
  "success_criteria_met": [
    { "id": "1", "description": "Research completed", "satisfied": true }
  ],
  "issues_encountered": [],
...
| `research_report.md` exists, >200 bytes | HARD | Must produce research report |
| `sources_count > 0` | HARD | Must cite at least one source |
| `confidence_overall < 30` | WARN | Very low confidence is flagged |

**If you cannot complete successfully**, set `status: "partial"` or `status: "failure"` with an honest explanation in `summary` and populate `issues_encountered`. Never write `status: "success"` if research is incomplete.
