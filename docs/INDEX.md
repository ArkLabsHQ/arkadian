...
**Tags**: `ark`, `protocol`, `server`, `vtxo`, `rounds`, `bitcoin`, `layer2`, `grpc`, `rest-api`, `postgresql`, `sqlite`, `redis`, `assets`, `teleport`, `fees`, `cel`, `client-lib`, `sdk`, `indexer`, `subscription`, `sweeper`, `startup-performance`, `admin-api`, `op-success-rejection`, `go-1.26.4`, `expired-rounds`, `wallet-utxos`, `effective-value-selection`, `settings`, `first-boot-seed`

**Synonyms**: `ark-server`, `arkd-server`, `ark-daemon`, `operator`

**Triggers**:
- **ask_question**: `vtxo`, `rounds`, `settlement`, `boarding`, `offchain`, `ark protocol`, `how does ark work`, `arkade assets`, `fees`, `teleport`, `expired rounds`, `wallet utxos`, `settings`, `env vars ignored`, `first boot seed`
- **develop**: `add endpoint`, `new database`, `migration`, `grpc service`, `round logic`, `asset`, `fee program`, `collected fees`, `admin fee report`, `expired rounds endpoint`, `wallet utxos endpoint`, `withdraw coin selection`, `settings endpoint`, `update settings`
...
- @arkade-os/sdk 0.4.34 and @arkade-os/boltz-swap 0.3.39 (PR #655 bump from 0.4.33 / 0.3.38; baseline established in PR #637 which moved pnpm `onlyBuiltDependencies` + `ignoredBuiltDependencies` from `package.json` into `pnpm-workspace.yaml`)
...
**Dependencies**: `@arkade-os/sdk` (0.4.34, JavaScript SDK), `@arkade-os/boltz-swap` (0.3.39), `@tanstack/react-virtual` (^3.13.19), `tailwindcss` (^4.2.2, with `@tailwindcss/vite`), `clsx` (^2.1.1), `tailwind-merge` (^3.5.0), `class-variance-authority` (^0.7.1), `sonner` (^2.0.7), `arkd` (server connection), `nostr-tools`
...
- Deploys arkd / arkd-wallet `v0.9.7` from GHCR (`ghcr.io/arkade-os/arkd*`, bumped in #87); Traefik upgraded to `v3.6.14`
...
- Deploys arkd / arkd-wallet `v0.9.7` from GHCR (`ghcr.io/arkade-os/arkd*`, bumped in #87 from `v0.9.6`)
...
- **X-Build-Version HTTP header** *(post-0.4.34)*: new `src/utils/fetch.ts` exports `buildVersion` (`"0.9.7"`) and a `fetch` wrapper that sets `X-Build-Version` on every outgoing request (throws if the Fetch API is unavailable). Consumed by `RestArkProvider` / `RestIndexerProvider` / `RestDelegateProvider` / `EsploraProvider`; `getExpoFetch` wraps the dynamically imported `expo/fetch` with the same header so React Native streaming requests carry it too. Lets the server (Arkana) correlate requests with the client build
