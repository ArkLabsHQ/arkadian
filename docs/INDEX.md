...
- Structured `SUBSCRIPTION_NOT_FOUND` indexer error code (PR #1140): every indexer path that touches a subscription (`GetSubscription`, `UpdateSubscription`, `SubscribeForScripts` reconnect, `UnsubscribeForScripts`) now routes a missing subscription through a single `subscriptionErr(id, err)` mapper (`internal/interface/grpc/handlers/indexer.go`) that emits the new `SUBSCRIPTION_NOT_FOUND` error `Code` (`pkg/errors/errors.go`, gRPC `NotFound`) carrying `SubscriptionMetadata{subscription_id}`, so SDKs can detect a stale subscription via the structured `ErrorDetails` code/name instead of parsing the error string. The message is restored to the legacy `subscription <id> not found` phrasing (PR #1074 had changed it to `subscription not found: <id>`, breaking string-matching SDKs — see ts-sdk#600), and the previously-unclassified `SubscribeForScripts` reconnect path — which returned `codes.Internal` with no structured code — now goes through the same mapper. Any non-not-found broker error still maps to `Internal`.

**Tags**: `ark`, `protocol`, `server`, `vtxo`, `rounds`, `bitcoin`, `layer2`, `grpc`, `rest-api`, `postgresql`, `sqlite`, `redis`, `assets`, `teleport`, `fees`, `cel`, `client-lib`, `sdk`, `indexer`, `subscription`, `sweeper`, `startup-performance`, `admin-api`, `op-success-rejection`, `go-1.26.4`, `expired-rounds`, `wallet-utxos`, `effective-value-selection`, `settings`, `first-boot-seed`, `signer-key-rotation`, `key-deprecation`, `sdk-version-header`, `client-deprecated-signer-verification`, `error-converter`, `grpc-status`, `nbxplorer-retry`, `startup-resilience`, `nbxplorer-reconnect`, `scanner-backoff`, `runtime-resilience`, `metadata-capping`, `offchain-tx-query`, `dependency-bump`, `http2`, `interceptor-order`, `readiness`, `digest-guard`, `channelz`, `grpc-introspection`, `permissions-whitelist`, `cursor-pagination`, `getvtxochain`, `page-token`, `checkpoint-watching`, `script-watching`, `replay-channel`, `batch-session`, `fee-rate`, `mempool-explorer`, `csv-closure`, `script-decoding`, `bip68`, `batch-trigger`, `round-gate`, `bip322`, `intent-proof`, `fee-estimation`, `min-relay-fee`, `nbxplorer-fallback`, `subscription-not-found`, `error-code`, `indexer-subscription`, `ts-sdk-600`
...
- Home & settings redesign (PRs #627–#664, #713): the bitcoin-only wallet home was rebuilt from the draft prototype — new `HomeHeader`, `PortfolioHero`, `HomeQuickActions`, `AssetsSection`, `UpsellsSection`, `RecentActivitySection` screens plus a redesigned full `Activity` page and receipt-style transaction detail, with shared motion (`src/lib/animations.ts`) and haptics (`src/lib/haptics.ts`) helpers. Bottom navigation is hidden for home-launched wallet flows; scan/swap route through a shared `SwapComingSoonSheet` drawer; the home asset card is tappable and only BTC + registered assets are listed. A new **Bitcoin detail page** (`src/screens/Wallet/BitcoinDetail.tsx`, PR #630) opened from the home asset row shows a Liveline price chart, current price, percentage badge, holdings, wallet actions, and bitcoin-filtered activity — driven by a new `src/lib/marketData.ts` (CoinGecko `max` data for non-USD all-time, sequential Coinbase all-time chunks, per-fiat/range cache, graph worker, one-week default, pause-on-hover, holdings guarded before BigInt). The root **Apps** surface was removed (PR #652): developer tools moved under **Settings → Advanced** (Boltz + Arkade Mint entries added), the main settings group renamed **Display**, and navigation tab ownership updated so these flows return through Settings. **Bitcoin currency units** (PR #653): BTC added as a selectable currency and display preferences replaced with bitcoin unit options (BTC / sats / BIP-177) applied across totals, transactions, send/receive, portfolio rows, and the Bitcoin detail view (detail market price stays fiat); fiat-like asset balances convert into selected-currency totals; falls back to BTC currency if the price feed fails. `config.fiat` was renamed to `config.currency` and `currencyDisplay` removed (PR #713; `Fiats` type → `Currencies`, legacy `'Sats only'` maps to `Currencies.BTC`) — supersedes earlier `config.fiat` references. The currency picker shows a circular bitcoin/country currency mark per option via reusable leading-content support on the settings `Select` + an extended `TokenLogo` (PR #663). The dev-mode Contracts screen also gained a full-width summary, clearer cards, copyable code fields, and subtle motion (PR #664).
- Minor UI/asset fixes: Arkade OG link-preview image swapped (PR #718, `public/arkade-og-image.png`; `index.html` metadata unchanged); header fix (PR #717); transaction-list height fix (PR #715); `regtest` submodule pointer bump (PR #716); `.env.regtest` gains an `AUTOMINE_INTERVAL` auto-miner knob.

**Tags**: `wallet`, `pwa`, `react`, `typescript`, `tailwindcss`, `design-tokens`, `sonner`, `shadcn`, `lucide`, `mobile`, `desktop`, `vtxo`, `lightning`, `boltz`, `lnurl`, `self-custodial`, `offline`, `indexeddb`, `nostr`, `playwright`, `chatwoot`, `announcements`, `arkade-regtest`, `service-worker`, `abortcontroller`, `bip39`, `bip86`, `mnemonic`, `taproot`, `pbkdf2`, `aes-gcm`, `signer-rotation`, `deprecated-signer`, `delegation`, `branta`, `build-version`, `node24`, `runtime-config`, `clipboard`, `hd-wallet`, `address-rotation`, `dev-mode`, `home-redesign`, `bitcoin-units`, `bip177`, `price-chart`, `market-data`
...
- Grafana dashboards (host metrics & cAdvisor segmented by `host_role`, rounds, VTXOs, transactions, performance) — includes a Prometheus-backed "Signer Key VTXO Usage" panel (`ark_signer_key_matched`, stacked bars by `pubkey`) tracking active VTXOs per signer key / key-rotation progress (PR #23), and an "Ark Channelz — Gateway Proxy" dashboard (`Ark_Channelz.json`, uid `ark-channelz-gateway`) tracking HTTP/2 stream utilization on the arkd gRPC gateway proxy from ark-metrics channelz metrics (`ark_channelz_unary_active_streams`, `ark_channelz_stream_pool_active_streams`; unary + per-stream-pool active streams and % of the 1000-stream budget) (PR #24)
...
**Tags**: `observability`, `monitoring`, `metrics`, `logs`, `traces`, `profiling`, `opentelemetry`, `prometheus`, `grafana`, `loki`, `jaeger`, `pyroscope`, `ec2`, `google-sso`, `channelz`, `grpc-gateway`
...
- Deploys arkd / arkd-wallet `v0.9.13` from GHCR (`ghcr.io/arkade-os/arkd*`, bumped in #107); Traefik upgraded to `v3.6.14`
...
- **ark-metrics** (`ghcr.io/arklabshq/ark-metrics:v0.2.0`, prod only, since #98; bumped `v0.1.0` → `v0.2.0` in #106) — collects Ark protocol metrics and exports them to the telemetry stack over OTLP to `otel-agent` (`ARK_METRICS_OTLP_ENDPOINT=http://otel-agent:4318`, insecure); `depends_on: [arkd, otel-agent]`, reads the arkd projection DB (`${ARKD_PG_DB_URL}`) and Ark info API (`https://${ARKD_DOMAIN}`); since #106 also scrapes arkd gRPC channelz (`ARK_METRICS_CHANNELZ_ENDPOINT=arkd:7071`, `ARK_METRICS_CHANNELZ_MAIN_PORT=7070`); `traefik.enable=false`; CloudWatch stream `ark-metrics`
...
- **Prod stack live (2026-05-26)**: new `apps/ark/prod/` OpenTofu entry point composes `modules/ark` (env=prod) — S3 backend `ark-prod-terraform-state` + DynamoDB `terraform-state-lock`, VPC/subnet lookup by `Name` tag (`ark-vpc-prod`, `ark-private-*`, `ark-public-*`), app instance `i-0f3d436aad5dbf55e`, `ark_infra_branch`/`ark_telemetry_branch = master`. Reachable at `arkade.computer` (primary ALB host since #104/#107) and `prod.arkade.sh` (direct A-record alias); Grafana at `telemetry.prod.arkade.sh`; `alb_log_retention_days = 30`. Prod-account Route53 hosted zone `prod.arkade.sh` added in `aws/prod-982590065524/route53.tf`. **ALB hostname migration (#104/#107)**: `arkd_hosts` is now `["arkade.computer", "prod.arkade.sh"]` (dropping `prod-cf.arkade.sh`); the primary ALB cert switched to a dedicated `arkade.computer` ACM cert (`f80fd08a-…`, provisioned in #104), while the prior `prod.arkade.sh`/`*.prod.arkade.sh`/`prod-cf.arkade.sh` cert (`57e4dfc4-…`) is retained as a temporary extra listener cert (`aws_lb_listener_certificate.tmp`, TODO removal) attached via the new `module.ark.alb_https_listener_arn` output (`modules/ark/outputs.tf`)
...
- Byte-slicing / conversion primitives (native emission): `substr(data, offset, size)` (`OP_SUBSTR`), `cat(a, b)` (`OP_CAT`), `bin2num(bytes)` (`OP_BIN2NUM`), `num2bin(num, size)` (`OP_NUM2BIN`), `size(bytes)` (`OP_SIZE OP_NIP`), and inline `sha256(data)` (`Expression::Sha256`) — combinable via the `byte_expr_comparison` grammar shape and accepted on the RHS of asset-lookup, group-property, and input/output introspection comparisons
- Packet introspection: `tx.packet(t)` (`OP_INSPECTPACKET`) and `tx.inputs[i].packet(t)` (`OP_INSPECTINPUTPACKET`) read attested packet bytes; `tx.inputs[i].arkadeScriptHash` / `arkadeWitnessHash` (`OP_INSPECTINPUTARKADESCRIPTHASH`) pins a consumed input to a contract; `this.activeBytecode` (`OP_INPUTBYTECODE`)
...
- LayerZero / USDT0 cross-chain suite (`examples/layerzero/`): four packet-native contracts — `endpoint.ark` (Endpoint state + receive/send, 2-of-2 DVN attestation via `checkSigFromStackVerify`, receive/send marker mint/burn), `oapp.ark` (USDT0 OApp state, mint/burn, marker consume/emit, recipient-output pinning to a `CreditMessage` slice), and `receive_marker.ark` / `send_marker.ark` (invocation markers pinned via `this.activeInputIndex` + `tx.inputs[i].arkadeScriptHash`). Expresses the full Go-script (`builders.go`) semantics on chain through `substr`/`bin2num`/`size` over `tx.packet(...)`
...
- Unified `IBitcoinBlockchain` interface (6 members: `GetChainTime`, `GetUtxosAsync`, `BroadcastAsync`, `BroadcastPackageAsync`, `GetTxStatusAsync`, `EstimateFeeRateAsync`) replaces the prior split `IBoardingUtxoProvider` / `IChainTimeProvider` / `IOnchainBroadcaster` trio. Three concrete impls under `NArk.Core/Blockchain/`: `NBXplorerBlockchain` (preserves cached-fallback chain-time + `submitpackage`/sequential broadcast fallback; `BroadcastPackageAsync` passes `maxfeerate=0` + `maxburnamount=21e6` to `submitpackage` and the sequential fallback broadcasts the CPFP parent via a direct-RPC `sendrawtransaction` with the same `maxburnamount` override so Ark P2A anchor outputs aren't rejected by Bitcoin Core's default burn-output check — PR #117), `EsploraBlockchain`, `RpcBlockchain` (`GetUtxosAsync` throws `NotSupportedException` — no native address index). DI helpers `AddNBXplorerBlockchain` / `AddEsploraBlockchain` / `AddRpcBlockchain` register every supported member of a backend in one call; `ArkApplicationBuilder.WithBlockchain<T>()` replaces the prior `WithTimeProvider<T>()`
...
- **Stale-subscription retry matches new arkd error format** *(0.4.42, `e0fac078`)*: `ContractWatcher` (`src/contracts/contractWatcher.ts`) resubscribes when it sends a subscription ID the server no longer recognises. The stale-detection regex broadens from `/subscription\s+\S+\s+not\s+found/i` to `/subscription\b.*\bnot\s+found/i` so it matches **both** arkd phrasings — the old `"subscription <uuid> not found"` and the new `"subscription not found: <uuid>"` — while all other errors (network failures, parse errors) still rethrow. Without the fix the new phrasing wasn't classified as stale, so the watcher rethrew instead of transparently creating a fresh subscription. Correctness-only, no public API change (`test/contracts/watcher.test.ts` reworked to cover both phrasings)
- **Sibling boltz-swap: invoice creation `timestamp` on decoded invoices** *(boltz-swap 0.3.47, `e9fd36ad`)*: `decodeInvoice` (`packages/boltz-swap/src/utils/decoding.ts`) now surfaces the BOLT11 `timestamp` section (Unix seconds of invoice creation) on `DecodedInvoice.timestamp`, and `ArkadeSwaps` forwards it onto `CreateLightningInvoiceResponse.timestamp` — so callers can compute the absolute expiry as `timestamp + expiry` (the `expiry` field is a **relative** delta in seconds from creation, not a Unix timestamp; the docstrings were corrected accordingly in `a104fb33`, and the type-checker `0` fallback documented as unreachable for a well-formed BOLT11 invoice in `ebc3c1d0`). Additive — no other surface change. Substantive change is in the sibling package — see `docs/projects/boltz-swap/`
- **0.4.42 release** *(`848be6a0`)*: release-only patch bump (`@arkade-os/sdk` 0.4.41 → 0.4.42, `@arkade-os/boltz-swap` 0.3.46 → 0.3.47). Publishes the two small changes above: the ts-sdk `ContractWatcher` stale-subscription error-format fix and the sibling boltz-swap invoice-creation-`timestamp` surfacing
- **`@marcbachmann/cel-js` bumped to 8.0.0 for MetaMask Snap (SES) compatibility** *(post-0.4.42 unreleased, `6f98fe02`, #602, closes #580)*: the `arkfee` `Estimator` pulls `@marcbachmann/cel-js` into every consumer bundle, and cel-js `< 8` shipped an interpreter method literally named `eval` — SES (run by `mm-snap build`) rejects any bare `eval(` token, so the SDK still couldn't bundle inside a MetaMask Snap even after `Estimator.eval()` → `.evaluate()` (#581). The upstream rename of cel-js's own method landed in **8.0.0** (7.6.1 still ships the bare `eval(`), so it's the lowest clearing version. The cel-js API the `Estimator` uses (`Environment`, `ParseResult`, `parseProgram`, `registerVariable`/`registerFunction`) is unchanged across the major bump (`estimator.ts` untouched) and 8.0.0's `node >=20.19.0` engine is already met. A guard test (`test/fee.test.ts`) asserts the resolved cel-js ships no bare `eval(` token so this can't regress — completing the SES-Snap saga started by the `eval` → `evaluate` rename
- **Regtest arkd image pinned to v0.9.11** *(post-0.4.42 unreleased, `618f055a`, #604)*: the `regtest/` submodule is fast-forwarded to arkade-regtest `master` (blesses **arkd v0.9.11**) and `packages/ts-sdk/.env.regtest` bumps its `ARKD_IMAGE` / `ARKD_WALLET_IMAGE` overrides `v0.9.9-rc.0` → `v0.9.11`. Regtest-fixture / CI only — no SDK source or public API change (the `buildVersion` header value stays `"0.9.9"`)
- **DIGEST_MISMATCH round-trip e2e across a real signer rotation** *(post-0.4.42 unreleased, `e023e1db`, #605)*: new `test/e2e/digestMismatch.test.ts` verifies the `X-Digest` / `DigestMismatchError` guard end-to-end against a live arkd server-signer rotation (mismatch observed → re-derive via `onServerInfoChanged` → throw rather than silently retry). Test-only coverage strengthening the Server-Signer Rotation feature (0.4.35, #554); no source or public API change

**Tags**: `typescript`, `sdk`, `wallet`, `vtxo`, `bitcoin`, `taproot`, `musig2`, `bip39`, `bip86`, `hd-wallet`, `hd-receive-rotation`, `wallet-receive-rotator`, `input-signer-router`, `input-router-classify`, `input-router-can-batch`, `batch-signable-identity`, `sign-multiple`, `one-popup-signing`, `combine-tapscript-sigs`, `descriptor-provider`, `wallet-mode`, `electrum`, `esplora`, `service-worker`, `service-worker-restore`, `react-native`, `expo`, `expo-background-task`, `metro-bundler`, `node-24`, `tsup`, `dist-smoke-test`, `storage-adapters`, `npm`, `mainnet-default`, `provider-default-urls`, `url-config-deprecated`, `dust-change-error`, `bigint-assets`, `vtxo-ownership-gating`, `unilateral-exit`, `refresh-outpoints`, `monorepo`, `pnpm-workspace`, `boltz-swap-sibling`, `package-scoped-releases`, `wallet-restore`, `gap-limit-discovery`, `discoverable-handler`, `scan-contracts`, `delegate-manager`, `delegator-delegate-rename`, `asset-manager-export`, `renew-vtxos-threshold`, `tapscript-memoization`, `transaction-history`, `boarding-dedup`, `boarding-handler`, `boarding-contract-type`, `ensure-wallet-contract`, `default-wins-coalescence`, `required-csv-timelock`, `indexer-spent-state`, `handler-authoring-helpers`, `is-csv-spendable`, `is-cltv-satisfied`, `regtest-node-cli`, `arkade-regtest`, `mempool-esplora`, `esplora-api-suffix`, `mempool-compat-blocks`, `mempool-compat-fee-estimates`, `bip21-validation`, `bip21-safe-integer`, `bip21-abnf-amount`, `btcd-taproot-tree`, `assemble-btcd-taproot-tree`, `max-vtxos-per-settlement`, `by-value-descending`, `by-expiry-ascending`, `recovery-cap-error`, `deprecated-signers`, `detect-used-scripts`, `scan-batch-size`, `default-page-size`, `vtxo-max-amount`, `cap-settlement-batch`, `amount-too-high`, `settle-rotation-race`, `x-build-version`, `build-version-header`, `missing-signing-descriptor-error`, `arkana`, `estimator-evaluate`, `ses-snap-compat`, `eval-rename`, `service-worker-init-guard`, `message-bus-initializing`, `worker-identity-assertion`, `electrum-get-merkle`, `normalized-error-text`, `description-hash`, `nip57-zap`, `bolt11-description-hash`, `bip322-intent-proof`, `psbt-global-signed-message`, `intent-proof-message`, `phantom-receive-fix`, `outspends-no-spender-txid`, `explorer-tx-vin`, `commitment-by-outpoint`, `wallet-clear`, `clear-local-data`, `indexeddb-onversionchange`, `indexeddb-delete-unblock`, `unsignable-boarding-input`, `boarding-signing-fast-fail`, `core-plugin-reuse`, `activity-history`, `build-activities`, `activity-registry`, `activity-resolver`, `boarding-resolver`, `get-activity-history`, `get-network-fail-closed`, `get-network-export`, `delegate-info-non-fatal`, `chain-swap-htlc-verification`, `assert-chain-htlc-leaves`, `verify-btc-chain-htlc`, `exact-claim-fee`, `target-fee-vsize`, `stale-subscription-retry`, `contract-watcher`, `invoice-timestamp`, `decode-invoice`, `bolt11-timestamp`, `cel-js-8`, `mm-snap-ses`, `no-bare-eval`, `arkd-0-9-11`, `regtest-arkd-pin`, `digest-mismatch-e2e`, `signer-rotation-e2e`
...
- **Contract Manager** (0.10.x) — every spendable output is modelled as a typed *contract*. `ark-core::contract` adds the shared model: `ContractType` (`default` / `delegate` / `boarding` / `vhtlc`), `ContractSpec`, prefixed vHTLC spend-path kinds, and centralized **contract spend selections** (`SpendSelection` / `SpendPathKind` + required spend control blocks). `ark-client::contract` adds the client-side `ContractManager` over a pluggable `ContractStore` trait (`MemoryContractStore` or `SqliteContractStore`, `new_default()`), a `ContractRegistry` of registered builtins, and annotation types (`AnnotatedVtxo`, `AnnotatedBoardingOutput`, `AnnotatedVtxoList`) that carry a stored contract + resolved spend selections/tapscripts/keys. **Boarding outputs moved into the contract manager and `BoardingWallet` was removed**; the `VtxoWatcher` and offchain flows now operate on annotated contract VTXOs. New client APIs: `Client::list_contracts() -> Vec<ContractInfo>` (wallet-facing contract views with derived address, server pk, and per-contract signer-rotation status) and `Client::restore_contracts(gap_limit) -> ContractRestoreReport` (contract-centric HD restore that scans derived keys, coalesces boarding + default contracts, and reports discovered/inserted/known contracts). Persisted contracts hydrate HD keys on connect (`hydrate_persisted_contract_keys`, without advancing the receive index) via the split-out `DiscoverableKeyProvider` (`with_discoverable_key_provider`); malformed builtin contract rows are surfaced rather than silently dropped. The `ark-client-sample` gains a `list-contracts` recipe, a `restore-contracts` command, and a configurable (memory / SQLite) contract-store backend (SQLite by default)
...
- **VTXO delegation**: 3-of-3 delegated VTXOs, REST delegator client (`ark-delegator`), background `VtxoWatcher` for auto-renewal (now also renews server-recoverable VTXOs and preserves the swept flag for delegated VTXOs)
...
**Tags**: `rust`, `sdk`, `ark`, `vtxo`, `musig2`, `grpc`, `rest`, `wasm`, `bdk`, `boltz`, `bitcoin`, `wallet-library`, `delegator`, `vtxo-watcher`, `arkade-asset`, `chain-swap`, `arkade-script`, `introspector-client`, `signer-rotation`, `contract-manager`, `contract-store`, `sqlite-contract-store`, `contract-spend-selections`, `annotated-vtxo`, `list-contracts`, `restore-contracts`, `discoverable-key-provider`, `boarding-contract`

**Synonyms**: `ark-rs`, `rust-ark-sdk`, `ark-rust`

**Triggers**:
- **ask_question**: `rust sdk`, `ark-rs`, `ark-core`, `ark-client`, `ark-delegator`, `ark-script`, `ark-introspector-client`, `rust wallet`, `wasm ark`, `bdk integration`, `vtxo watcher`, `arkade asset rust`, `rust chain swap`, `arkade script rust`, `contract manager rust`, `contract store`, `sqlite contract store`, `list contracts`, `restore contracts`, `annotated vtxo`, `discoverable key provider`
- **develop**: `add rust feature`, `new crate`, `ark-core type`, `musig2 signing`, `wasm support`, `e2e test`, `delegator client`, `asset issuance rust`, `chain swap rust`, `arkade tapscript`, `introspector packet`, `forfeit unilateral exit key`, `signer rotation`, `migrate deprecated signer`, `deprecated signer status`, `contract manager`, `contract spend selections`, `contract store backend`, `annotate vtxos`, `restore contracts`, `hydrate persisted keys`
...
| rust-sdk | arkd | Client-Server (via gRPC/REST, SDK 0.10.1) |
