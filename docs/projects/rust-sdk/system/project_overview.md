...
- **`ark-bdk-wallet` on-chain descriptors switched to BIP86 Taproot** — `Wallet::new_from_xpriv` now derives its external and change descriptors from the `Bip86` BDK template (P2TR, `bc1p…` addresses) instead of `Bip84` (P2WPKH native SegWit, `bc1q…`). New on-chain boarding/exit addresses are Taproot, aligning the Rust on-chain wallet with the Taproot-first posture of the sibling SDKs. Note: wallets rebuilt from an existing `Xpriv` (e.g. a BIP39 mnemonic) will surface a different, Taproot address set for the same seed.
...
- `Wallet::new_from_xpriv` builds **BIP86 Taproot** (P2TR) external/change descriptors — on-chain addresses are `bc1p…` (previously BIP84 native SegWit `bc1q…`)
