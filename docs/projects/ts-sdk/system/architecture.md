...
```
packages/ts-sdk/src/
├── index.ts                 # Main exports (~400 lines of re-exports). Since 0.4.33 also re-exports `isCsvSpendable` / `isCltvSatisfied` from `contracts/handlers/helpers.ts` so authors of custom `ContractHandler` implementations can reuse the canonical BIP-68 / BIP-65 timelock-maturity logic the built-in handlers already use to implement `selectPath` / `getSpendablePaths`
