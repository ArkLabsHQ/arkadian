...
The parser uses [pest](https://pest.rs/), a PEG (Parsing Expression Grammar) parser generator for Rust. The grammar is defined in `grammar.pest` (752 lines); AST construction is split across per-concern submodules (`expr`, `comparison`, `checksig`, `crypto`, `asset`, `introspection`, `tapscript`) coordinated by `parser/mod.rs`. It covers:
...
4. **Bytes-aware `+` rewrite pass**: walk each function bottom-up with a `Scope` and rewrite every `BinaryOp { op: "+" }` whose left or right operand resolves to a bytes-like type into `Expression::Concat`, recording per-side `coerce_left` / `coerce_right` flags so int operands get `OP_SCRIPTNUMTOLE64` before `OP_CAT`. Pure `int + int` stays as `OP_ADD64`. The rewrite (in `compiler/concat.rs`) uses `typechecker::infer_type()` (and `Scope` / `build_scope`, now `pub(crate)` since the pipeline modules were made crate-internal) so emission can keep its lowering logic uniform.
...
The crate is a Cargo **workspace** (`members = [".", "arkade-bindgen"]`). The compiler is the root package; the pipeline modules (`parser`, `compiler`, `typechecker`, `validator`) are **crate-internal** (`mod`, not `pub mod`) — only `compile()`, `models`, and `opcodes` are exported. Since the 2026-07 refactor, `parser/` and `compiler/` are split into per-concern submodules instead of monolithic `mod.rs` files.

```
src/
├── main.rs              # CLI: parse args, read .ark, compile, write JSON
├── lib.rs               # Public API: compile(source_code) → Result<ContractJson>;
│                        # re-exports models + opcodes; pipeline modules are crate-internal
├── wasm.rs              # WASM bindings for the web playground (behind the `wasm` feature)
├── parser/              # PEG → AST, split by concern
│   ├── mod.rs           # build_ast(), parse_contract(), parse_function(), tapscript parsing
│   ├── grammar.pest     # PEG grammar: 752 lines
│   ├── expr.rs          # expression parsing
│   ├── comparison.rs    # comparison / byte_expr_comparison parsing
│   ├── checksig.rs      # checkSig / checkMultisig / checkSigFromStack parsing
│   ├── crypto.rs        # hash fns, ecMulScalarVerify, tweakVerify
│   ├── asset.rs         # asset lookup / group / control-predicate parsing
│   ├── introspection.rs # tx / input / output / packet introspection parsing
│   └── tapscript.rs     # `function … tapscript {}` leaf parsing (NamedTapscript/TapItem)
├── models/
...
├── compiler/            # AST → unified ABI, split by concern
│   ├── mod.rs           # compile(), covenant emission, bytes-aware `+` rewrite orchestration
│   ├── expr.rs          # expression ASM emission
│   ├── comparison.rs    # generate_comparison_asm (postfix / RPN operand order)
│   ├── concat.rs        # bytes-aware `+` → OP_CAT rewrite + OP_SCRIPTNUMTOLE64 coercion
│   ├── loops.rs         # compile-time `for` loop unrolling (substitute_loop_body)
│   ├── asset.rs         # asset-ID collection / decomposition, asset & group emission
│   ├── introspection.rs # tx / input / output / packet introspection emission
...
├── typechecker/
│   └── mod.rs           # ArkType-based expression typing (Scope/build_scope, now pub(crate))
└── opcodes/
    └── mod.rs           # OP_* opcode constants (extracted from compiler)

arkade-bindgen/          # Workspace member: Go/TypeScript client-binding generator
├── src/
│   ├── lib.rs, main.rs  # CLI + library entry
│   ├── artifact.rs      # parse compiled ContractJson
│   ├── ir.rs            # IR over the spend-groups/leaves ABI (AbiFunctionGroup{arkade, leaves[]})
│   ├── naming.rs        # identifier / type naming
│   └── targets/         # go.rs, typescript.rs emitters (per-leaf witness types,
│                        # buildWitness(group, leaf, w))
└── tests/               # ir_test, go_test, typescript_test + fixtures/ (regenerated JSON)
```

## Testing Architecture

Integration tests live in two aggregator binaries — `tests/examples.rs` (contracts compiled from `examples/`) and `tests/features.rs` (language/compiler behaviour via inline snippets) — each pulling in per-topic modules from `tests/examples/*.rs` and `tests/features/*.rs` via `#[path = "..."] mod ...`. Shared helpers (`asm_of`, `asm_variant`, `witness_names`, `opcode_count`, `user_signatures`) live in `tests/common/mod.rs`, included into both binaries the same way. The old parser-only `tapscript_parse_test` was inlined as a `parser` unit test. Run the whole suite with `cargo test`, or a binary with `cargo test --test examples` / `cargo test --test features`; filter by module name (e.g. `cargo test --test features bare_vtxo`). CI runs `cargo build/clippy/test --workspace` so the `arkade-bindgen` member can't silently break.

**`tests/examples/` (contract compilation):**
- `htlc`, `fuji_safe`, `stability_vault` (oracle-signed settlement, no-oracle invariants on `transfer`/`split`, OP_CAT + OP_SHA256 message reconstruction), `covered_call` / `cash_secured_put` (Rysk-faithful single-locked options: exercise/reclaim CLTV windows, transfer pre-expiry guard), `repayment_pool` / `bond_mint` (fixed-maturity bond market: phased-lifecycle time gates, strict-burn equality, deployment-invariant assertions, ceiling-division origination floor, force-liquidation co-spend regression guards), `arkade_kitties`, `token_vault`, `controlled_mint`, `fee_adapter`, `threshold_oracle`
- `asm_structural` (BSST-style structural checks) and `compilation_roundtrip` (compile-then-re-parse parity across example contracts) also run over the example set

**`tests/features/` (language / compiler behaviour):**
- **Contract fixtures**: `bare_vtxo`, `beacon`, `threshold_multisig`
- **Tapscript leaves**: `tapscript_validation` (closure-shape / arkd-rule enforcement), `tapscript_abi` (unified group/leaf ABI shape, default-leaf synthesis), `tapscript_golden` (golden parity of HTLC leaves against arkd closures)
- **Introspection**: `asset_introspection`, `tx_introspection`, `io_introspection`, `packet_primitives` (byte-slicing `substr`/`cat`/`bin2num`/`num2bin`/`size`, packet introspection, `byte_expr_comparison`)
- **Cross-chain**: `layerzero` (LayerZero / USDT0 suite: 2-of-2 DVN attestation via `OP_CHECKSIGFROMSTACKVERIFY`, packet-native receive/send, marker mint/burn via group sums, `arkadeScriptHash` input pinning)
- **New opcodes**: `new_opcodes`, `concat_op` (type-dispatched `+`: bytes-vs-int dispatch, OP_SCRIPTNUMTOLE64 coercion, pure int+int stays OP_ADD64)
- **Asset groups**: `group_properties` (`controlIs`/`hasControl` predicates)
- **Asset IDs**: `asset_id_explicit` (explicit `(txid, gidx)` operands; compile-time operand-type/range validation)
- **Validation & structure**: `validation_error` (AST/output validator errors), `no_shadowing` (immutable-ctor-param assignment rejection, scope-shadowing and `for (x, x)` detection, emitted-namespace collisions), `type_system` (typechecker behaviour), `contract_import_instantiation` (cross-contract imports), `epoch_limiter`
