...
- **Mnemonic identity (PR #624)**: New wallets create a `MnemonicIdentity` (BIP86 Taproot derivation at `m/86'/coinType'/0'/0/0`, coinType `0` mainnet / `1` testnet) from a 12-word mnemonic; the mnemonic is encrypted at rest in `localStorage` via PBKDF2 (100k iters, SHA-256) + AES-GCM with a per-record salt + IV. `Restore` auto-detects 12-word mnemonics vs `nsec1...` / raw hex input. `Settings → Backup` shows the recovery phrase for mnemonic wallets and the `nsec` for legacy wallets. `Settings → Password` re-encrypts the mnemonic (not just the legacy private key) on password change. Wallet mode is a persisted `Config` field (`walletMode: 'static' | 'hd'`, default `'static'`) — as of PR #682 mnemonic (HD-capable) wallets can opt into `'hd'` address rotation, while legacy `SingleKey` wallets remain `'static'` (see **HD Address Rotation** below). The Nostr backup key for mnemonic wallets is the BIP86-derived signing key (deliberate — same key serves both Nostr backup and Ark signing, matching legacy raw-privkey behaviour).
...
- **Home & settings redesign (PRs #627–#664, #713)**: Bitcoin-only wallet home rebuilt from the draft prototype (new `HomeHeader`/`PortfolioHero`/`HomeQuickActions`/`AssetsSection`/`UpsellsSection`/`RecentActivitySection` + redesigned Activity page and receipt-style tx detail), with shared motion (`src/lib/animations.ts`) and haptics (`src/lib/haptics.ts`) helpers. A new **Bitcoin detail page** (`BitcoinDetail.tsx`, PR #630) adds a Liveline price chart (via new `src/lib/marketData.ts` — CoinGecko/Coinbase data, graph worker, per-fiat/range cache, one-week default), current price, percentage badge, holdings, and bitcoin-filtered activity. The root **Apps** surface was removed (PR #652) — dev tools moved to **Settings → Advanced** (Boltz + Arkade Mint entries), the main settings group renamed **Display**. **Bitcoin currency units** (PR #653): BTC is a selectable currency and display prefs become BTC / sats / BIP-177 unit options applied across totals/transactions/send/receive/portfolio/detail (detail market price stays fiat). `config.fiat` was renamed `config.currency` and `currencyDisplay` dropped (PR #713; `Fiats` type → `Currencies`). The currency picker shows per-option bitcoin/country marks via `TokenLogo` (PR #663).
...
- **Plumbing**: `initWallet` and `initSvcWorkerWallet` gain `walletMode` + `restoring` params; `FlowContext.InitInfo` and `WalletContextProps.initWallet` carry `walletMode`. On mnemonic init the resolved mode is written back via `updateConfig({ ...config, pubkey, walletMode })`.
- **Restore recovery**: when `restoring` is true, after wallet boot the service worker runs `svcWallet.restore()` (status "Recovering addresses…") to gap-scan and recover rotated HD addresses; failures are logged via `consoleError` but non-fatal.
- **Dev-mode selectors**: triple-tapping the onboarding "Welcome to Arkade 👾" heading (`data-testid='onboarding-devmode-tap'`) toggles dev mode before a wallet exists. With dev mode on, **Init** shows a "Rotate receive addresses" `Toggle` in a create-wallet `SheetModal` (`hd`/`static`), and **Restore** shows an Inherit/Static/HD `SegmentedControl` for mnemonic input (`ROTATION_TO_MODE`: Inherit → `undefined` so `resolveWalletMode` inherits the Nostr-backup-restored `config.walletMode`). Non-dev users always create `static`. Covered by `walletMode.test.ts`, `Init.test.tsx`, `Restore.test.tsx`.

### Developer / Diagnostics (PR #618, PR #617, PR #670, PR #674)
- **Dev mode**: Triple-tapping the loading logo (or, since PR #682, the onboarding welcome heading) toggles a global dev mode persisted in `localStorage`. The tap logic was lifted out of `LoadingLogo` into a new `DevModeProvider` (`src/providers/devMode.tsx`), so every loading logo in the app shares the same state.
...
- **@arkade-os/sdk** (0.4.42, PR #714) for Ark protocol operations (incl. ts-sdk PR #554 signer-rotation classification: `signerSetFromInfo`, `classifyAgainstSignerSet`; the `DelegateInfo` type used by the delegation flow; also exports `buildVersion` / `sdkVersion` surfaced in the Support screen's Chatwoot attributes)
- **@arkade-os/boltz-swap** (0.3.47, PR #714) for Lightning swap integration (incl. submarine recovery API; `arkade-money` referralId on swap provider + arkadeSwaps; optimistic `waitForSwapFunded` consumed by the live-settlement Lightning send in PR #668, plus `waitFor: 'funded'` + preimage backfill from 0.3.41); its `sdkVersion` is surfaced as the `boltz_swap_version` Chatwoot attribute (PR #691)
...
**Dependencies**: @arkade-os/sdk 0.4.42, @arkade-os/boltz-swap 0.3.47, @branta-ops/branta 3.1.3
