...
project_id: wallet
version: 1.2.41
last_sync_commit: e14de0733b325e81cddce2bfeef16e4c416ffe66
...
- **Delegation accepts deprecated signer keys** (PR #708, `src/screens/Settings/Delegates.tsx` / `src/lib/asp.ts`): the delegate connection test (`testConnection`) no longer validates the delegate server's advertised key against only the current `aspInfo.signerPubkey`. It now builds a `possibleXOnlyPubkeys` set from the current signer plus every `aspInfo.deprecatedSigners` entry whose `cutoffDate > now` (each normalised to x-only by stripping a leading byte when 66 hex chars), and accepts the delegate if the decoded `serverPubKey` matches **any** of them — so a delegate still keyed on a not-yet-expired deprecated signer passes validation during a cooperative key rotation. Connection errors are now logged via `consoleError('Error testing delegate connection:')` instead of being swallowed. In `delegateVtxos` (`asp.ts`), `dm.getDelegateInfo()` (returning the SDK's new `DelegateInfo` type) is wrapped in try/catch — a fetch failure logs via `consoleError` and returns early instead of throwing — and the partial-delegation-failure `console.warn` was replaced with `consoleError('Delegation partial failure:')`.
- **Absent-delegate-URL handling** (PR #731, `src/lib/constants.ts` / `src/providers/wallet.tsx` / `src/screens/Settings/Delegates.tsx`): networks with no default delegate (`DELEGATE_URL[network] === null`, e.g. testnet) previously threw `Delegate URL not found for network` when the delegation settings screen or wallet init reached for the delegate. `getDelegateUrlForNetwork` now returns `string | undefined` instead of throwing, and a new `getDelegateForNetwork` returns `Delegate | undefined`. The **Settings → Delegates** screen renders a "No delegate found for this network." `WarningBox` (hiding the toggle, warning, and delegate card) when no delegate URL is configured; `testConnection` resolves `undefined` and `DelegateCard` renders nothing until a delegate is confirmed. `WalletProvider` init passes the raw URL-or-undefined through as `delegatorUrl` instead of dereferencing `.url` on a guaranteed object.
...
### Home & Settings Redesign (PRs #627–#664, #713)
- **Redesigned bitcoin home (PR #627/#628)**: Bitcoin-only wallet home rebuilt from the draft prototype — new `HomeHeader`, `PortfolioHero`, `HomeQuickActions`, `AssetsSection`, `UpsellsSection`, and `RecentActivitySection` screens (`src/screens/Wallet/`), plus a redesigned full `Activity` page and receipt-style transaction detail. Shared motion (`src/lib/animations.ts`) and haptics (`src/lib/haptics.ts`) helpers added. Bottom navigation is hidden for home-launched wallet flows; scan/swap interactions wire through a shared swap coming-soon drawer (`src/components/SwapComingSoonSheet.tsx`). The home activity preview stays hidden when empty (full Activity page refreshes before showing an empty state); the home asset card is tappable to open the asset. Only BTC and registered assets are shown on the homepage. BigInt coercions are guarded against render-time crashes.
- **Bitcoin detail page (PR #630, `src/screens/Wallet/BitcoinDetail.tsx`)**: Opened from the home asset row — Liveline price chart, current price, percentage badge, holdings, wallet actions, and bitcoin-filtered recent activity. New `src/lib/marketData.ts` fetches chart data (CoinGecko `max` data for non-USD all-time ranges, Coinbase all-time chunks fetched sequentially), caches responses per fiat/range, runs through a graph worker, defaults to a one-week period, and pauses the chart while hovering; the holdings balance is guarded before BigInt conversion.
- **Advanced tools moved into Settings (PR #652)**: The root **Apps** surface is removed — developer-focused tools now live under **Settings → Advanced** (Boltz and Arkade Mint entries added there), the main settings group was renamed to **Display**, and navigation tab ownership was updated so these flows return through Settings instead of an Apps tab.
- **Bitcoin currency units (PR #653)**: **BTC** added as a selectable currency; display preferences replaced with bitcoin unit options (**BTC**, **sats**, **BIP-177** notation). Wallet totals, transactions, send/receive flows, portfolio rows, and the Bitcoin detail view render in the selected bitcoin unit (the detail market price stays in fiat); fiat-like asset balances are converted into selected-currency totals; falls back to BTC currency if the price feed fails.
- **Brazilian Real (BRL) fiat currency (PR #743)**: `Currencies.BRL` added to `src/lib/types.ts` and wired through the fiat pipeline — `FiatPrices` gains a `brl` field, `getPriceFeed` reads `json.BRL?.last`, the `FiatProvider` (`src/providers/fiat.tsx`) adds `fromBRL`/`toBRL` conversions plus BRL branches in `fromFiatAmount`/`toFiatAmount`, and `FIAT_SYMBOLS` maps BRL to the `R$` prefix. **Settings → Currency** lists BRL alongside the existing options (BTC/CHF/CNY/EUR/GBP/JPY/USD). Covered by `src/test/screens/settings/fiat.test.tsx`.
- **Unlock-splash / service-worker registration fix (PR #752)**: `src/index.tsx` was drastically simplified — the dev-only wallet-storage/service-worker reset helpers (`resetDevWalletStorage`, `resetControlledDevServiceWorker`, `reloadAfterUnregisteringServiceWorkers`, `getDevWalletStorageResetKey`) and the manual `register()` + `controllerchange` reload dance were removed in favour of a single `updatefound` listener on the existing registration that reloads the page when a new service worker appears, fixing race conditions in the unlock splash on service-worker updates. `src/wallet-service-worker.ts` was cleaned up alongside.
- **`config.fiat` → `config.currency` (PR #713)**: Config field `fiat` renamed to `currency`, `currencyDisplay` removed from config, and the `Fiats` type renamed to `Currencies` (supersedes earlier `config.fiat` references, incl. the Send "Max" fiat-decimals path). The config migration maps a legacy `currencyDisplay: 'Sats only'` to `Currencies.BTC`.
...
- **BTC amounts shown to 8 decimals (PR #777, `src/lib/format.ts` / `src/components/InputAmount.tsx`)**: `prettyAmount`'s ≥100 BTC and ≥1 BTC satoshi branches now format via `prettyNumber(fromSatoshis(sats), 8, true, 8)` (full 8-decimal min/max padding) instead of 0 / 3 fraction digits — `prettyAmount(150000000)` → `1.50000000 BTC` (was `1.5 BTC`). `formatBitcoinUnitAmountParts` defaults both min/max fraction digits to `amount === 0 ? 0 : 8`, so a zero balance renders `0 BTC` while any non-zero value pads to 8 decimals (`prettyBitcoinAmount(2100, Unit.BTC)` → `0.00002100 BTC`). `InputAmount`'s fiat-mode BTC "other value" likewise pads to 8 decimals via `prettyNumber(btcValue, decimals, true, decimals)`. Covered by `src/test/lib/format.test.ts`.
- **Tabs concept removed from navigation (PR #782, `src/providers/navigation.tsx`)**: the `Tabs` enum and the `pageTab` page→tab mapping were deleted, along with the `tab`/`setTab` state and the `tab` field on `NavigationContext` (consumers no longer receive it). Navigation-animation direction now derives solely from `ROOT_PAGES` membership (`isFromRoot`) rather than a same-tab comparison, and `isInitialLoad` simplifies to `screen === Pages.Wallet`.
