...
**Checkpoint Sweep Event Propagation (PR #1155):** When `createCheckpointSweepTask` sweeps a checkpoint, it now emits a `TransactionEvent{Type: SweepTxType, SweptVtxos: childrenVtxos}` via the `onSweepCheckpoint` callback (wired to `service.propagateTransactionEvent` in `NewService`). This notifies clients subscribed to the transaction event stream (`GetTxEventsChannel`) and the indexer of the checkpoint sweep tx — previously only round-level sweeps surfaced sweep events, so checkpoint sweeps were silent to subscribers.
...
- `GetSubscription()` - Server-streaming RPC. If `subscription_id` is empty the server creates a subscription inline, applies the optional initial `SubscriptionFilter`, and emits a `SubscriptionStartedEvent` carrying the generated id as the first message; if `subscription_id` is set the stream attaches to the existing listener (legacy flow). Bound to both `/v1/indexer/script/subscription/{subscription_id}` (legacy) and `/v1/indexer/subscription` (single-connection). **Single-consumer displacement (PR/commit #5c56d54c):** each stream becomes the subscription's sole consumer via a broker `attach()`/`release()` pair — attaching cancels any pending removal timeout and closes the `displaced` channel of the previously attached stream, forcing the old stream to exit so it can't steal events meant for its replacement. On exit `release()` keeps the listener alive for the reconnect window only if it still has scripts or tx filters, otherwise removes it. Newly-created (inline) subscriptions attach with a zero reconnect window; reconnecting streams use `subscriptionTimeoutDuration`. The dispatch loop uses two intentional selects so a pending exit signal is handled before draining `listener.ch`, leaving buffered events for the successor.
...
**Structured tx-filter errors (PR #1141):** invalid or over-cap tx filters now surface structured codes from `pkg/errors` (both gRPC `InvalidArgument`) instead of opaque errors:
- `TX_FILTERS_LIMIT_EXCEEDED` (code 51) — carries `TxFiltersLimitMetadata{subscription_id, max_tx_filters, got_tx_filters}` when a subscription's CEL `expressions` list exceeds the per-subscription cap.
- `INVALID_TX_FILTER` (code 52) — carries `TxFilterMetadata{expression}` when a CEL tx-filter expression fails to compile/parse, identifying the offending expression.
