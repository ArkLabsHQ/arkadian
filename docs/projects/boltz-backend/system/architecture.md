...
- **LndClient**: gRPC client for LND — pinned Docker image **v0.21.1-beta** (bumped from `v0.21.0-beta`, PR #1452 / `v0.20.1-beta`, PR #1432; `VersionCheck` range bumped to `minimal=0.19.0` / `maximal=0.21.1`; vendored `router.proto` refreshed with clarified HTLC-interceptor idempotency / on-chain resolution semantics). The Rust `boltzr` sidecar's `lnd::describe_graph` (added in PR #1424) exposes LND's channel/node gossip to the new `GraphLightningInfo` aggregator alongside CLN's `listchannels` / `listnodes`, so Lightning gossip in `service::lightning_info` is sourced from both backends (the previous CLN-only `ClnLightningInfo` was replaced).
...
- **ArbitrumProvider** (`lib/wallet/ethereum/ArbitrumProvider.ts`, PR #1456): the swap lockup locktime on Arbitrum is denominated in the **L1** block height, which Arbitrum exposes as `l1BlockNumber` on each block. The provider now reads `l1BlockNumber` (falling back to `number`) directly from its own latest block via `getLatestBlock`, instead of maintaining a separate `l1Provider` (`InjectedProvider` against Ethereum mainnet) — the removed `arbitrum.l1Providers` config array is replaced by a single boolean `arbitrum.regtest` flag. When an RPC returns no `l1BlockNumber` and `regtest` is not set, the provider **throws** rather than silently falling back to the L2 block number (only anvil/regtest forks legitimately omit it); `regtest = true` permits the L2 fallback. `init()` also performs an eager `getLatestBlock()` so a misconfigured RPC fails fast at startup instead of being swallowed by the block poller. Block-number parsing switched to `ethers.getNumber` to also handle the decimal `l1BlockNumber` anvil returns.
