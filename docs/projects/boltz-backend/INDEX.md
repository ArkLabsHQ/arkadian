...
- **EVM submarine-swap refund-address index** (schema **v28**, PR #1468): the Sequelize migration (`case 27`) adds a nullable `refundAddress` (`STRING(255)`) column to the `swaps` table plus a partial index `swaps_refundAddress` (`WHERE refundAddress IS NOT NULL`), mirrored in the `Swap` model. `EthereumNursery` back-fills `refundAddress` from the `EtherSwap`/`ERC20Swap` lockup event (via `SwapRepository.setRefundAddress`) so an EVM submarine swap can now be recovered through `/v2/swap/restore` by the EVM address the client refunds to — the restore-by-EVM-address lookup now matches **both** the claim and refund addresses. `Migration.latestSchemaVersion` bumped `27 → 28`.
...
- **BOLT12**: Support for offers and blinded paths (hardened). The `POST /v2/lightning/{currency}/bolt12/fetch` endpoint now makes `amount` **optional** (PR #1469): when the offer embeds an amount in the chain's native (bitcoin) unit the server coalesces it from the offer, so clients no longer have to decode the offer and echo its amount back; omitting the amount for an offer that carries no bitcoin-denominated amount returns `422`. The Rust `assert_not_zero` deserializer now yields `Option<u64>` (accepts absent/`null`, still rejects `0`).
