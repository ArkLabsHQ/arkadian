...
- `PreimageDerivationTests.cs` -- PR #116 cross-SDK pin on `SwapsManagementService.BuildPreimageMessage`. Three tests: `Message_IsTagPlusXOnlyPubkeyPlusIndexLittleEndian` (canonical byte layout `"Arkade-Boltz-Preimage-v1" || xonly_pubkey(32) || u32_le(index)`), `Message_IndexEncodedAsUInt32LittleEndian` (`index: 1` encodes as `0x01 0x00 0x00 0x00`), and the load-bearing `Message_IsIndependentOfDescriptorStringForm` — proves an HD signing descriptor like `tr([d34db33f/86'/0'/0']xpub.../0/*)` and the bare receiver descriptor a restore reconstructs `tr(<x-only hex>)` produce the same message because both share the same derived x-only key. Without that invariant a restored wallet would re-derive a different preimage than the original create-time descriptor produced and the VHTLC claim would fail
