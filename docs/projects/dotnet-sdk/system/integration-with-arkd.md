...
| `SubscribeForScriptsAsync(scripts, subscriptionId)` | Creates a new subscription (when `subscriptionId == null`) or **adds** scripts to an existing one. Returns the subscription id. arkd's subscription is mutable while streaming — scripts added here are routed onto the already-open stream (PR #103) |
| `UnsubscribeForScriptsAsync(subscriptionId, scripts)` | Removes scripts from an existing subscription (or all, tearing it down server-side, when `scripts == null`) (PR #103) |
| `GetVtxoSubscriptionStreamAsync(subscriptionId)` | The long-lived server stream. The subscription's script set may be mutated in place via `Subscribe`/`Unsubscribe` while this stream stays open (PR #103) |
| `GetVtxoToPollAsStream` | One-shot convenience default-method built on the three primitives above. Subscribes for the given script set, opens the stream, and yields. Does **not** surface the subscription id — prefer the primitives when the watched set changes over time |
...
- **In-place script-set updates** (PR #103) — the service keeps one long-lived `GetSubscription` stream open through a supervisor loop (`RunStreamSupervisorAsync`) and mutates the watched set by issuing `SubscribeForScriptsAsync(added, _subscriptionId)` / `UnsubscribeForScriptsAsync(_subscriptionId, removed)` deltas rather than tearing the stream down and recreating it on every change. arkd routes the added/removed scripts onto the already-open stream, so the teardown/recreate window (where a pushed event could slip through) is gone. The supervisor reconnects on the same `_subscriptionId` if the stream drops; if arkd returns "subscription not found" (TTL-GC'd the listener after a longer disconnect), `UpdateScriptsView` recreates it with `SubscribeForScriptsAsync(newView, subscriptionId: null)`, signals the supervisor via `SignalStreamGenerationChange`, and enqueues a full-history catch-up poll for the recreated set. An empty active set tears the subscription down (`UnsubscribeForScriptsAsync(id, scripts: null)`) and the supervisor goes idle on `_streamWakeup` until scripts return. `StreamReconnectDelay = 1s` backoff between transient stream faults; the stream supervisor logs `Warning` on a drop and reconnects.
