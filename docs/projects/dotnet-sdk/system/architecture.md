...
`VtxoSynchronizationService` keeps **one long-lived arkd `GetSubscription` stream** open through a supervisor loop (`RunStreamSupervisorAsync`) and mutates the watched script set **in place** (PR #103). When `UpdateScriptsView` sees a delta, it issues `SubscribeForScriptsAsync(added, _subscriptionId)` / `UnsubscribeForScriptsAsync(_subscriptionId, removed)` rather than tearing the stream down. The supervisor reconnects on the same id; if arkd reports the subscription not found (TTL-GC'd after a longer disconnect), the service recreates it with a null `subscriptionId`, signals the supervisor via `SignalStreamGenerationChange`, and enqueues a full-history catch-up poll for the recreated set. An empty active set unsubscribes everything and the supervisor goes idle on `_streamWakeup` until scripts return. `StreamReconnectDelay = 1s` between transient stream faults. The 5-second safety-net `RoutinePoll` re-derives the active script set **fresh** from `IActiveScriptsProvider`s every tick via `GatherActiveScriptsAsync` (PR #102) and reconciles the subscription to that set — a stale or missed `ActiveScriptsChanged` event can never hide a script from detection. A failing provider is logged and skipped (per-provider isolation). `EfCoreVtxoStorage.GetActiveScripts` overrides the default with a `DISTINCT script` projection, so the per-tick derive stays cheap even for wallets with thousands of unspent VTXOs. `_lastViewOfScripts` was renamed `_subscribedScripts` and demoted to pure stream-subscription bookkeeping (it tells the supervisor when to recompare; it is no longer the source of truth for what to poll).
