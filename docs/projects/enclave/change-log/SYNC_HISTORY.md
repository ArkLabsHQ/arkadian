...
**Synced By**: /update-project enclave
**Status**: Documentation updated — `enclave tofu` split into `init` / `update` / `env` subcommands; interactive S3+DynamoDB backend bootstrap via the bundled `modules/backend` submodule; runtime env overlay switched from `ENCLAVE_APP_ENV_KEYS` baked-in key list to `GetParametersByPath` scan of `/<dep>/<app>/env/`; `app.env` reframed as opt-in for PCR0-attested build-time values only.

**Commits Analyzed** (3):
- `19f967a` refactor(cli): update CLI commands and comments to reflect 'enclave tofu init' usage — `enclave tofu` becomes a subcommand group; `tofu init` replaces the old single-shot scaffold. `runTofuScaffold` → `runTofuInit`; new `resolveEnclaveConfig` factored out (PWD-rooted config discovery, honours `ENCLAVE_CONFIG`, then flat `./enclave.yaml`, then `./enclave/enclave.yaml`). All inline strings, generated CI workflow (`test/app/.github/workflows/deploy-enclave.yml`), and doc comments updated to `enclave tofu init`. `test/run.sh` invokes `enclave tofu init`. `cli/build.go`'s end-of-build "Next:" hint dropped (the lifecycle is no longer linear). `cli/cli_test.go` + `cli/tofu_test.go` updated.
- `cf7abd8` feat(cli): implement backend bootstrapping and interactive prompts for S3 and DynamoDB configuration — new `cli/prompt.go` with `isTTY` (via `golang.org/x/term`), `yesNo`, and `promptWithDefault` helpers. New `cli/tofu_bootstrap.go::bootstrapBackend` runs `tofu init` + `tofu apply -auto-approve` on the `tofu/modules/backend` submodule (creates the S3 state bucket + DynamoDB lock table); stdio inherited so operators see progress live; BucketAlreadyExists surfaces an actionable hint with the import command. `cli/tofu.go::writeBackendConfig` now writes `tofu/backend.tf` independently of the bootstrap decision (skip-if-exists so operator customizations survive), parameterized by a new `backendOverride { bucket, table, region }` with `defaultBackendValues(cfg)` computing the defaults from `enclave.yaml`. `cli/tofu_scaffold.go::tofuInitCmd` adds `--bootstrap-backend` / `--no-bootstrap` / `--backend-bucket` / `--backend-table` / `--backend-region` flags + a `collectBackendValues` resolver that overlays flags onto prompted/default values (anyFlag → skip prompt; local mode / empty `cfg.Account` / pre-existing `backend.tf` / non-TTY → skip prompt). New `tofuUpdateCmd` + `runTofuUpdate` refresh `terraform.tfvars.json` from `enclave.yaml` (modules + `backend.tf` untouched; requires existing `tofu/main.tf`). `go.mod` adds `golang.org/x/term`.
- `fc6a60c` Remove enclave module and update test run script for env_values handling — removes the legacy build-time `ENCLAVE_APP_ENV_KEYS` mechanism end-to-end. `runtime/environment.go::applyEnvOverrides` now scans `/<dep>/<app>/env/` via `GetParametersByPath` (paginated, `WithDecryption: true`, `Recursive: false`) and overlays every key it finds onto the process env; defensively skips empty / nested keys. `runtime/aws_clients.go::SSMAPI` + the `ssmGetter` test interface add `GetParametersByPath`. `cli/framework_files.go` removes `appEnvKeysJson` + the `ENCLAVE_APP_ENV_KEYS=...` line from all four `flake.nix` variants (Go / Node.js / .NET / Rust). `cli/init.go` and `cli/template.go` scaffold `app.env` as `env: {}` with new comments steering operators to `enclave tofu env`. New `cli/tofu_env.go::tofuEnvCmd` introduces `enclave tofu env --key K --value V [...]` (`envKeyRe = ^[A-Z_][A-Z0-9_]*$`, sorted-stable JSON write, merge-preserves prior keys, requires existing `tofu/main.tf`). `test/run.sh` now drives `enclave tofu env` instead of writing hand-crafted JSON into `env_values.auto.tfvars.json`. `test/app/tofu/.gitignore` adds the CLI-managed scaffold paths (`main.tf`, `modules/enclave/main.tf`, `env_values.auto.tfvars.json`); the previously-committed `test/app/tofu/main.tf`, `test/app/tofu/modules/enclave/main.tf`, and `test/app/tofu/env_values.auto.tfvars.json` are deleted, along with the redundant `test/app/enclave/flake.nix`. `runtime/environment_test.go` rewritten against the new `GetParametersByPath` semantics; `cli/tofu_test.go` extended (`+157` LOC).

**Documentation Updates**:
- `INDEX.md` (project) — CLI Commands lifecycle table: replaced the single `enclave tofu [--remote]` row with three rows covering `enclave tofu init [--remote]` (incl. the new backend-bootstrap flags), `enclave tofu update [--remote]`, and `enclave tofu env --key K --value V`.
- `system/project_overview.md` — "Build-time vs deploy-time env" capability bullet rewritten to describe the SSM-scan model (`GetParametersByPath`), the removal of `ENCLAVE_APP_ENV_KEYS`, and `app.env` being opt-in for PCR0-bound build-time values; new "`enclave tofu` subcommand group" capability bullet; "Two artifact-source modes" bullet's command updated to `enclave tofu init --remote`.
- `system/architecture.md` — new "Deploy-time Env Overlay (SSM-scan)" section right before the file map, documenting `Environment.Override`, the `GetParametersByPath` scan, defensive key filtering, the `SSMAPI` interface change, and the `app.env` reframing.
- `testing/usage.md` — Deploy section reworked around the new `tofu init` / `tofu env` flow (incl. the backend-bootstrap prompt note); `app.env` paragraph rewritten for the SSM-scan model + `enclave tofu env` UX; ACME redeploy snippet updated to `enclave tofu update && (cd tofu && tofu apply)`.
- `testing/how_to_run.md` — local workflow step 4 expanded to cover all three `tofu` subcommands; new paragraph after the workflow explains the `init` / `update` / `env` split + backend-bootstrap flags + the SSM `GetParametersByPath` overlay; `TF_VAR_env_values` row in the env-vars table annotated to point at the new CLI flow.
- `testing/api-reference.md` — small fix: "When `enclave tofu` is applied …" → "When `enclave tofu init` has scaffolded the module and `tofu apply` runs …".
- `INDEX.md` (master) — enclave Key Capabilities: replaced the old "Build-time vs deploy-time env split" bullet with an SSM-scan-model rewrite; added a new "`enclave tofu` subcommand group" bullet; updated the "Two artifact-source modes" bullet to reference `enclave tofu init --remote`. Tags extended with `tofu-init`, `tofu-update`, `tofu-env`, `backend-bootstrap`, `dynamodb-lock-table`, `ssm-env-overlay`, `getparametersbypath`. Triggers extended with `enclave tofu env`, `tofu env_values`, `deploy-time env`, `ENCLAVE_APP_ENV_KEYS removed`, `GetParametersByPath`, `enclave tofu subcommands` (ask_question); `backend bootstrap`, `tofu_bootstrap`, `tofu_env`, `bootstrapBackend`, `defaultBackendValues`, `writeBackendConfig`, `Environment.Override`, `app.env` (develop); `enclave tofu init`, `enclave tofu update`, `enclave tofu env` (test_or_run).
