...
│   │   ├── scripts/user-data-telemetry.sh # Bootstrap: writes /etc/ark/telemetry-bootstrap.yml + runs the local ansible/telemetry.yml (slimmed 2026-07, #117; was SSM-token Ansible bootstrap)
│   │   ├── telemetry.tf → aws_ec2_subnet_cidr_reservation + static aws_service_discovery_instance # fixed-IP Cloud Map registration (since #117); provisioning moved to top-level ansible/telemetry.yml (old modules/ark/ansible/telemetry-playbook.yml deleted)
...
├── packer/                            # AMI builds (base since #102; subdirs + bitcoin-node in #105; AL2023 in #115; telemetry in #117) — packer/base/ (→ ark-base-al2023-arm64-<ts>, Graviton-only), packer/bitcoin-node/ (Bitcoin Core 29.0 → ark-bitcoin-node-<ver>-al2023-arm64-…), packer/telemetry/ (Docker+Compose → ark-telemetry-al2023-arm64-…), packer/Makefile (make ami-base / ami-bitcoin-node / ami-telemetry)
├── ansible/                           # Playbooks + roles — site.yml (base image: baseline/cloudwatch_agent/ansible_runtime/deprovision), bitcoin-node.yml + roles/bitcoind, and telemetry.yml + roles/telemetry (since #117). Shared roles: docker (Compose v2 plugin), ebs_data_volume, fixed_eni_ip (used by bitcoind + telemetry); ansible_runtime installs the generic ark-converge@ template unit. Runs at Packer build and idempotently on live hosts via /opt/ark/ansible
...
- **Telemetry instance (separate EC2, ASG, fronted by shared ALB)** — provisioned by `modules/ark/telemetry.tf` and bootstrapped by the top-level `ansible/telemetry.yml` playbook (since #117; previously `modules/ark/ansible/telemetry-playbook.yml`). Hosts:
...
App ↔ telemetry resolution uses **AWS Cloud Map** private DNS. **Since #117** the registration is **static**: a reserved fixed private IP (`aws_ec2_subnet_cidr_reservation` + required `telemetry_fixed_private_ip`) is bound as a secondary IP on the primary ENI at boot and a static `aws_service_discovery_instance` points Cloud Map at it — replacing the old boot-time `servicediscovery:RegisterInstance`/deregister self-registration (and its shutdown deregister unit).
...
**Graviton + AL2023 AMI pattern (2026-07, #117):** telemetry adopts the standalone-bitcoin-node provisioning pattern — a custom **Amazon Linux 2023 arm64 (Graviton)** AMI built by the new `packer/telemetry/telemetry.pkr.hcl` (`make ami-telemetry`), default instance `t4g.small` (was Ubuntu/x86 `t3.small`), root device `/dev/xvda`. The monolithic `modules/ark/ansible/telemetry-playbook.yml` (+ `requirements.yml`) is replaced by the top-level `ansible/telemetry.yml` playbook running roles `docker` (installs the Compose v2 plugin `2.29.7` aarch64, which AL2023 omits), `telemetry`, `ansible_runtime`, and build-only `deprovision`; the `ebs_data_volume` and `fixed_eni_ip` roles are now **shared** with the bitcoind role. A generic **`ark-converge@` template systemd unit** (`systemctl start ark-converge@telemetry`) replaces the per-AMI converge units. Config/secrets are fetched from SSM at converge time and the ark-telemetry stack is cloned live at `ark_telemetry_ref`. `user-data-telemetry.sh` shrinks to writing `/etc/ark/telemetry-bootstrap.yml` + running the local playbook (no apt/pip/awscli install, no repo clone). Telemetry IAM drops `servicediscovery:Register/Deregister/ListInstances` for `ec2:AssignPrivateIpAddresses`/`UnassignPrivateIpAddresses` (VPC-scoped) + `ec2:DescribeNetworkInterfaces`; the telemetry-specific CloudWatch drop-in is removed (baseline already covers host + `/mnt/data`; a second drop-in crash-looped the agent). `telemetry_ami_id` and `telemetry_fixed_private_ip` are now required. Staging: `t4g.small`, `ami-0644e3471d063291b`, fixed IP `10.10.102.12`.
